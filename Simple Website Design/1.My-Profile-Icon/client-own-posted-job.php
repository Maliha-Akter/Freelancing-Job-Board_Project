<?php
session_start();
include("job-posting-db.php");

// Check if the form is submitted
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['deleteJobId'])) {
        $deleteJobId = (int)$_POST['deleteJobId'];

        // Ensure that the job being deleted belongs to the logged-in client
        $client_id = (int)$_SESSION['user_id'];
        $checkOwnershipQuery = "SELECT * FROM Job WHERE JobId = $deleteJobId AND client_id = $client_id";
        $checkOwnershipResult = mysqli_query($conn, $checkOwnershipQuery);

        if ($checkOwnershipResult && mysqli_num_rows($checkOwnershipResult) > 0) {
            // Job belongs to the logged-in client, so it can be deleted
            $deleteJobQuery = "DELETE FROM Job WHERE JobId = $deleteJobId";
            $deleteJobResult = mysqli_query($conn, $deleteJobQuery);

            if (!$deleteJobResult) {
                echo "<p>Error deleting job: " . mysqli_error($conn) . "</p>";
            }
        } 
        // else {
        //     echo "<p>You don't have permission to delete this job.</p>";
        // }
    }
}

?>

<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <style>
        body {
            font-family: Arial, sans-serif;
            background-color: #f4f4f4;
            margin: 0;
            padding: 0;
            display: flex;
            justify-content: center;
            align-items: center;
            height: 100vh;
        }

        .job-container {
            max-height: 400px; /* Adjust the maximum height based on your preference */
            overflow-y: auto;
            /* margin-top: 1100px; */
            width: 400px;
            background-color: #fff;
            padding: 20px;
            border-radius: 8px;
            box-shadow: 0 0 10px rgba(0, 0, 0, 0.1);
        }

        h2 {
            text-align: center;
            color: #333;
        }

        .job-details {
            margin-top: 20px;
        }

        .job-details p {
            margin: 5px 0;
        }

        .job-details hr {
            border: 0.5px solid #ddd;
        }

        .delete-btn {
            background-color: #ff5252;
            color: #fff;
            padding: 5px 10px;
            border: none;
            border-radius: 4px;
            cursor: pointer;
        }
    </style>
</head>

<body>

    <div class="job-container">
        <h2>Your Posted Jobs</h2>

        <?php
        // Check if a user is logged in
        if (!isset($_SESSION['user_id'])) {
            echo "<p>User not logged in</p>";
            exit;
        }

        // Fetch client_id from the session
        $client_id = (int)$_SESSION['user_id'];

        // Retrieve all jobs for the logged-in client
        $jobQuery = "SELECT * FROM Job WHERE client_id = $client_id ORDER BY JobId DESC";
        $jobResult = mysqli_query($conn, $jobQuery);

        // Check for SQL errors
        if (!$jobResult) {
            echo "<p>Error: " . mysqli_error($conn) . "</p>";
            exit;
        }

        // Display job details
        while ($row = mysqli_fetch_assoc($jobResult)) {
            echo "<div class='job-details'>";
            echo "<p>Job ID: " . $row['JobId'] . "</p>";
            echo "<p>Job Category: " . $row['JobCategory'] . "</p>";
            echo "<p>Required Skills: " . $row['RequiredSkills'] . "</p>";
            echo "<p>Working Hour: " . $row['WorkingHour'] . "</p>";
            echo "<p>Project Size: " . $row['ProjectSize'] . "</p>";
            echo "<p>Budget: $" . $row['Budget'] . "</p>";
            echo "<button class='delete-btn' onclick='deleteJob(" . $row['JobId'] . ")'>Delete</button>";
            echo "<hr>";
            echo "</div>";
        }
        ?>

        <script>
            function deleteJob(jobId) {
                // Confirm before deleting the job
                if (confirm("Are you sure you want to delete this job?")) {
                    // Create a form and submit it to handle the deletion
                    var form = document.createElement("form");
                    form.method = "post";
                    form.action = "<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>";
                    var input = document.createElement("input");
                    input.type = "hidden";
                    input.name = "deleteJobId";
                    input.value = jobId;
                    form.appendChild(input);
                    document.body.appendChild(form);
                    form.submit();
                }
            }
        </script>

    </div>

</body>

</html>
